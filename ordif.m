function [u,h,Time]=ordif(Num,eps,h,T,Time,e)
%a=Num %b=eps, c=h(1)d=h(2) dt=e , f=T, g=time
Num=1; % define the number of layers(Num+1 layers)
% input from switch
...
for k=1:T;
    d=abs(f(h(1),h(2),eps)); % general variable for time adaptivity
    if e==1% general variable for time adaptivity
    dt=1/(10000*d);
    elseif e==2;
        dt=Settime(d);
