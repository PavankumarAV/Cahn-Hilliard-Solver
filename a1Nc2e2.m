function [h, Time]=a1Nc2e2(N,eps,e)
Num=N;
...
for k=1:T;
    ...
    if e==1
        dt=Settime(d);
    elseif e==2% general variable for time adaptivity
    dt=1/(10000*d);
...
if h(1)>=h(2)-2*eps;
    plot(xp,up);
title(['ODE cannot capture collpase, try switching scheme',' Total Time is ',num2str(Time),' s' ])

    break
    elseif h(1)<=2*eps;
         plot(xp,up);
title(['ODE cannot capture collpase, try switching scheme and',' Total Time is ',num2str(Time),' s' ])
    % making dependent on eps
     break
elseif h(2)>=h(3)-2*eps;
     plot(xp,up);
title(['ODE cannot capture collpase, try switching scheme and',' Total Time is ',num2str(Time),' s' ])

     break
elseif h(3)>=h(4)-2*eps;
    title(['ODE cannot capture collpase, try switching scheme and',' Total Time is ',num2str(Time),' s' ])
